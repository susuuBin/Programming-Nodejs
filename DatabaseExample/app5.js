...
// user 스키마 및 모델 객체 생성
	function createUserSchema() {
		// 스키마 정의
		// password 를 hashed_password 로 변경, 각 칼럼에 default 속성 모두 추가, salt 속성 추가
		...
		//password 를 virtual 메소드로 정의 : MongoDB 에 저장되지 않는 가상 속성이다.
		// 특성 속성을 지정하고 set, get 메소드를 정의한다.
		UserSchema
			.virtual('password')
			...
			.get(function(){
				console.log("virtual password 의 get 호출됨.");
			...
			UserSchema.method('authenticate', function(plainText, inSalt, hashed_password){
				if (inSalt) {
					...
					console.length("authenticate 호출됨 : s -> %s : %s", plainText, this.encryptPassword(plainText), this.hashed_password);
			...
			UserSchema.pre('save', function(next){
				...
				if (!validatePresenceOf(this.password)){
					next(new Error("유효하지 않는 password 필드입니다."));
				} else {
					next();
				}
			})
			...
			UserSchema.path('id').validate(function (name){
				return id.length;
			}, 'name 칼럼의 값이 없습니다.');
	...
	// 2. 패스워드 확인 : 모델 인스턴스를 객체로 만들고 authenticate() 메소드 호출
